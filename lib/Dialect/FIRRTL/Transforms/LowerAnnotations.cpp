...
LogicalResult LowerAnnotationsPass::solveWiringProblems(ApplyState &state) {
  ...
  for (auto [index, problem] : llvm::enumerate(state.wiringProblems)) {
    ...
    for (auto source : sources) {
      auto mod = cast<FModuleOp>(source.getReferencedModule());
    ...
    for (auto sink : sinks) {
      auto mod = cast<FModuleOp>(sink.getReferencedModule());
      moduleModifications[mod].portsToAdd.push_back(
          {index,
           {StringAttr::get(
                context, state.getNamespace(mod).newName(problem.newNameHint)),
  ...
  for (auto *op : llvm::post_order(instanceGraph.getTopLevelNode())) {
    ...
    for (auto [index, portInfo] : modifications.portsToAdd) {
      // Create the port.
    ...
    for (auto [index, portPair] : llvm::zip(problemIndex, newPorts)) {
    ...
    for (auto [index, dest] : moduleModifications[fmodule].uturns) {
      Value src = moduleModifications[fmodule].connectionMap[index];
    ...
    for (auto *inst : instanceGraph.lookup(fmodule)->uses()) {
      ...
      // this is a U-turn.
      for (auto [index, value] : llvm::enumerate(problemIndex)) {
        if (moduleModifications[enclosingModule].connectionMap.count(value)) {
          moduleModifications[enclosingModule].uturns.push_back(
              {value, clonedInst.getResult(index + originalNumPorts)});
          continue;
        }
        moduleModifications[enclosingModule].connectionMap[value] =
            clonedInst.getResult(index + originalNumPorts);
      }
    }
  }
